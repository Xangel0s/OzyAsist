...
- **Interfaces**:
  - `cmd/ozy`: CLI / TUI interactiva basada en Bubble Tea con menú de inicio retro (años 80-90) navegable con flechas y soporte de mouse (`↑`/`↓`/`Enter`: Iniciar conversación, Historial de conversaciones, Configuraciones, Salir), pantalla de historial de chats persistidos en SQLite con reanudación y borrado interactivo (`d` + confirmación), panel de diagnóstico BIOS, transición fluida al chat agéntico moderno con encabezado horizontal de espacio de trabajo (`OzyAssist: Plan & Workspace`), tarjetas interactivas de decisión y plan (`InteractiveCard`) con navegación por pestañas y flechas, barra de entrada con badge de marca `[OZY]` en Neon Lime `#d1f107`, comandos `/new` y `/history`, streaming, pensamiento colapsable (`Ctrl+T`), colas de mensajes, gestor interactivo de claves API, editor visual de perfil de usuario en SQLite, soporte nativo de mouse (`tea.WithMouseCellMotion`: scroll en viewport y clics en menús/tarjetas), recorte estricto anti-scroll en terminal y superficies atenuadas sin emojis.
