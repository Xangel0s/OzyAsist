...
- **Interfaces**:
  ...
  - `cmd/ozy voice`: Consola autónoma de voz dedicada e interactiva (`RunVoiceConsole`) con espectro de sonido animado en vivo (`[ESCUCHANDO]` / `[OZY HABLANDO]`), interrupción instantánea con `Esc` o `/cancel` (*Barge-in*), síntesis local por oraciones en tiempo real con Windows SAPI / Piper, y barra de entrada de texto interactiva para escribir órdenes directamente por teclado cuando el usuario lo prefiera.
