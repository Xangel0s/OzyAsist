...
- **Interfaces**:
  - `cmd/ozy`: CLI / TUI interactiva basada en Bubble Tea con menú de inicio retro (años 80-90) navegable con flechas y soporte de mouse (`↑`/`↓`/`Enter`: 1. Iniciar conversación, 2. Historial de conversaciones, 3. Configuraciones, 4. Modo Voz en Vivo, 5. Salir; o teclas rápidas `1`-`5`, `v` para voz inmediata), acceso instantáneo a **Modo Voz** dentro del chat mediante el atajo **`Ctrl+V`** o comando `/voice` (despliega la barra lateral con el espectro animado de ondas en vivo `[ESCUCHANDO]` / `[OZY HABLANDO]`, escucha continua con Wake Word 'Hey Ozy' y síntesis local en tiempo real con Windows SAPI / Piper sin abrir otra terminal), pantalla de historial de chats persistidos en SQLite con buscador en tiempo real, reanudación y borrado interactivo (`d` + confirmación), panel de diagnóstico BIOS, transición fluida al chat agéntico moderno directo con encabezado sobrio de sesión (`#shortID · <Tema>`), **Ventana Lateral de Contexto Interactiva (Sidebar)** colapsable con `Ctrl+B` o `/context` (telemetría de tokens vs límite de modelo con barra de progreso, métricas de RAM en Go puro `runtime.MemStats`, conteo de proyectos en RAM y recuerdos activos en SQLite), **Gestor Visual Interactivo de Memorias** (`/memmgr`, `/memories gui`), **Chat Refinado de Alto Contraste** (usuario en gris neutro `#888888`, agente con encabezado `O Ozy:` en Neon Lime `#d1f107`), **Playground Plegado por Defecto** (`showThinking: false`, desplegable con `Ctrl+T` o `/thinking`), barra de estado inferior no redundante, auto-limpieza de sesiones vacías en SQLite, barra de entrada con badge `[OZY]` en Neon Lime `#d1f107`, comandos `/new` y `/history`, colas de mensajes y soporte nativo de mouse (`tea.WithMouseCellMotion`).
  - `cmd/ozy voice`: Comando directo de CLI que lanza la TUI en la misma terminal iniciando directamente en Modo Voz con el micrófono en escucha activa y la barra lateral animada abierta.
